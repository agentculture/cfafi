...
- the cite-don't-import pattern (which cultureflare uses: patterns from
  afi-cli are copied into this repo and adapted, not installed).

## cultureflare's place in the org

cultureflare is the first concrete product built using afi-cli's patterns.
...
- Every PR bumps the version in `pyproject.toml` (+ `cultureflare/__init__.py`
  - `CHANGELOG.md`).
