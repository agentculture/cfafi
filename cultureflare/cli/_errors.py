...
Every failure inside cultureflare raises :class:`CfafiError`. The top-level
``main()`` catches it, formats via :mod:`cultureflare.cli._output`, and exits
...
class CfafiError(Exception):
    """Structured error raised within cultureflare; carries a remediation hint.
