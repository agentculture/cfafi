"""``cultureflare learn`` — self-teaching prompt for agent consumers.
...
-----------------
  ...
  cultureflare dns create ZONE TYPE NAME CONTENT [--apply]
                                  Create a DNS record. Dry-run by default.
  cultureflare learn                     This prompt. Supports --json.
  cultureflare explain <path>...         Markdown docs for any noun/verb path.

Credentials
-----------
cultureflare reads two environment variables and never touches the filesystem:
...
Secure loading pattern (recommended):
  ...
  2. In your agent script, source it just before invoking cultureflare:
...
-----------
  ...
  cultureflare explain zones list
  cultureflare explain dns create

Homepage: https://github.com/agentculture/cultureflare
"""
...
def _json_payload() -> dict[str, object]:
    return {
        ...
        "secure_loading": (
            ...
            "'set -a; . /path/to/cultureflare.env; set +a' before invoking cultureflare"
        ),
        "json_support": True,
        "dry_run_policy": "mutations default to dry-run; pass --apply to commit",
        "explain_pointer": "cultureflare explain <path> (e.g. 'cultureflare explain dns create')",
    }
